...
\todo{Übergänge + Jahreszahlen?}

Many researchers adopted the \pp metric in their work in the following years, further underlining the necessity of such an objective performance portability metric. \todo{cite \pp usage papers here?}
...
For example, \citet{harrell_effective_2018} noted that \pp tracks performance and portability but completely ignores the third P, productivity. 
...
The work of \citet{daniel_applying_2019} combines the \pp metric of \citet{pennycook_evaluating_2018} with the code divergence of \citet{harrell_effective_2018}. 
...
\citet{sedova_high-performance_2018} also created a new performance portability metric measuring the performance portability on a programmatic level since they also argue that \pp does not penalize non-portable but highly optimized code paths. 
...
\citet{yang_empirical_2018} also tried to improve \pp but not by changing or augmenting \pp itself but by proposing a new performance efficiency measurement in addition to the already existing architectural and application efficiency. 
They argue that the architectural efficiency may skew the results because it uses vendor-provided theoretical and possibly over-optimistic peak performance values. 
...
\end{definition}
However, the Roofline model and, therefore, the Roofline efficiency, suffer two major problems~\cite{bertoni_performance_2020, marowka_reformulation_2022}. 
...
Second, empirically created Roofline models, e.g., using the stream benchmark to determine the sustained memory bandwidth, may give different results on different machines even though the same hardware is used. 
...
\citet{yokota_beginners_2018} analyzed the \pp metric using five case studies and highlighted some findings and problems, namely the tension between performance improvements and performance portability improvements. 
...
Adding new platforms for already supported types with a good efficiency will improve the \pp value. 
...
Improving performance efficiency on a platform can have multiple effects: improving efficiency on all platforms (best case) or improving efficiency only on this platform but decreasing it on all other platforms (worst case). 
\citeauthor{yokota_beginners_2018} also highlight the architectural efficiency problem resulting in a good efficiency score, although the implementation may not be efficient. 
...
Besides \citeauthor{yokota_beginners_2018}, \citet{marowka_toward_2021}, with an extended paper published at the beginning of 2022~\cite{marowka_reformulation_2022}, also thoroughly investigated the \pp metric and found some issues, the main one being the usage of the harmonic mean in the \pp \autoref{def:pp}. 
...
Second, he observed that in many real-world examples, there is a noticeable gap between the performance efficiency of \glspl{cpu} and \glspl{gpu} if \glspl{cpu} are even supported. 
He, therefore, suggests that \mpp should only be calculated for platforms in the same architectural class, i.e., calculate different \mpp values for \glspl{cpu} and \glspl{gpu}. 
...
While some of these changes can be justified, the question of whether points two and four are desirable is especially pertinent if the goal is to showcase performance portability across diverse architectures. 

In \cite{marowka_performance_2022}, \citeauthor{marowka_performance_2022} also proposed a variation of his \mpp metric, which is not application but programming model-centric. 
...
Since \citeauthor{marowka_portability_2024} also criticized the architectural and application efficiencies, he also defined a new performance portability metric to tackle some of these problems~\cite{marowka_portability_2024}. 
...
He also proposes to create \enquote{Performance Portability Repositories} containing the optimal implementations for various different problems that can then be used as baselines for the application efficiency calculations. 
...
The basic idea of the \textit{portability efficiency} describes the performance cost of a one-way migration of an application from a source to a target platform. 
