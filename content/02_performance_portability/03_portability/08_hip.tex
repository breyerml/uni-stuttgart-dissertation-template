...
The first public release of \gls{hip} is from March 2016~\cite{advanced_micro_devices_release_2016} and the current version 6.3.3 was released in February 2025~\cite{advanced_micro_devices_release_2025}. 
While \gls{hip}'s main target is \gls{amd} \glspl{gpu}, it also supports NVIDIA \glspl{gpu} from the beginning~\cite{advanced_micro_devices_its_2016} using NVIDIA's own compiler stack.
...
The \gls{hip} \gls{api} is extremely close to the one provided by NVIDIA: in most cases, conversions between \gls{cuda} and \gls{hip} can be achieved via a simple regex for more complex cases, the HIPIFY~\cite{advanced_micro_devices_rocmhipify_2025} tool can be used. 
