...
These implementations include but are not limited to \gls{amd}, Intel, NVIDIA, LLVM, GNU GCC, ARM, etc. 
...
The actual parallelization in \gls{openmp} is done through the compiler pragmas. 
These pragmas have the benefit that they are easy to use and non-intrusive. 
Additionally, since these are only pragmas if a compiler does not support \gls{openmp}, he is free to ignore them. 
This means that code parallelized with \gls{openmp} can also be compiled with compilers that do not support it, granted that no runtime library functions are used, but as a result, the executable also only runs single-threaded. 
Internally, \gls{openmp} uses a fork-join model where an initial thread spawns a number of so-called work threads in a parallel region. 
...
\autoref{code:openmp_example} shows a simple DAXPY example using \gls{openmp} and \gls{openmp} target offloading. 
...
Additionally, \gls{openmp} also supports task parallelism, which is not shown here since it will not be used in the remainder of this thesis. 
\autoref{code:openmp_example} also shows a \gls{openmp} target offloading example of the same code at the bottom. 
While the first code can only be executed on \glspl{cpu}, the target offloading code can also be executed on any supported \gls{gpu}. 
However, the downside of \gls{openmp}'s high-level directive-based constructs is that low-level performance optimizations are generally impossible, resulting in worse performance than hand-tuned native codes. 
