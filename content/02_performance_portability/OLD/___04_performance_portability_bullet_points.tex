...
% Given different platform categories $\mathbf{C}$, and the set of all platforms $\mathbf{H}$ where each $\mathbf{p \in H}$ is associated with a platform category $\mathbf{c \in C}$, let the heterogeneity score be defined as:
...
% $$p_c = \begin{cases}
%             \frac{\abs{\{ p \in H \mid p \text{ is associated with } c \}}}{\abs{H}} & \text{if at least one platform is associated with } c \\
%             1 & \text{otherwise}
%         \end{cases}$$ 

% is the ratio of the number of platforms in the current category relative to the total number of platforms. 
% \end{definition}
