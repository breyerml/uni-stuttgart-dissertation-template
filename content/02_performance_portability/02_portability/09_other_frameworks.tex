...
All previously mentioned libraries, frameworks, or programming languages will be used in the remainder of this dissertation, at least to some extent. 
...
Over one year before the release of the first \gls{openmp} specification, including target offloading capabilities, in 2013, the first \gls{openacc} specification was released in November 2011~\cite{openacc_standard_openacc_2011, hutchison_openacc_2012}, and the most recent one in November 2022~\cite{openacc_standard_openacc_2022}. 
The basic idea of \gls{openacc} is nearly identical to \gls{openmp}'s target offloading. 
Both use a directive-based approach with optional library functions. 
As with \gls{openmp}, different implementations, e.g., from NVIDIA or the GNU GCC compiler, exist. 
...
Alpaka was released in May 2016~\cite{zenker_alpaka_2016, alpaka-group_release_2016} and is based on a master's thesis from~\citet{worpitz_investigating_2015}. 
The current version is v2024.07.0, released in June 2024~\cite{llnl_release_2024}. 
Its design is comparable to that of Kokkos and can also target all major hardware platforms using one of the provided backends: \gls{openmp}, \gls{cuda}, \gls{hip}, or SYCL. 

RAJA was also first released in 2016~\cite{llnl_release_2016}, with the current version 1.2.0 released in October 2024~\cite{alpaka-group_release_2024}. 
...
It supports various hardware platforms through execution policies for \gls{openmp} (target offload), \gls{cuda}, \gls{hip}, or SYCL. 
