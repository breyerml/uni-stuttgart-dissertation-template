...
\begin{quotebox}{\href{https://github.com/kokkos/kokkos}{Kokkos GitHub README}}
    Kokkos Core implements a programming model in \cpp for writing performance portable applications targeting all major HPC platforms. For that purpose it provides abstractions for both parallel execution of code and data management. Kokkos is designed to target complex node architectures with N-level memory hierarchies and multiple types of execution resources.
\end{quotebox}
...
As with SYCL, parallelism can be expressed differently in Kokkos and interoperated with specific backend libraries. 
...
\begin{table}[]
    ...
    \caption{%
        ...
        Currently, six execution spaces are fully supported, while three remain experimental.  
        Nevertheless, this table shows that Kokkos supports all major hardware platforms with different execution spaces. 
    }
    \label{tab:kokkos_execution_spaces}
\end{table}

\autoref{tab:kokkos_execution_spaces} shows the available Kokkos execution spaces. 
...
The overall host-to-device offload code structure \gls{cuda} or SYCL. 
However, as already mentioned, the abstraction level is a different one: the memory allocations are hidden behind the \code{Kokkos::View} (lines 4+5), and the targeted device is hidden behind a Kokkos execution space which is in the provided example the implicit \code{Kokkos::DefaultExecutionSpace} since none was explicitly provided.  
The actual kernel is similar to the one in SYCL written using a simple lambda function, whereby we have to note that normal \cpp lambda functions are not enough, but the Kokkos specific \code{KOKKOS_LAMBDA} macro must be used. 
Due to the \code{Kokkos::View} abstraction, no manual resource cleanup must be done at the end of our code. 
