...
However, \citet{alpay_sycl_2020} showed with their AdaptiveCpp (back then known as hipSYCL) implementation, which directly uses \gls{openmp}, \gls{cuda}, and \gls{hip} to target the respective hardware that this tight integration with \gls{opencl} is unnecessary and even hinders the future directions of SYCL. 
...
\begin{table}[]
    ...
    \caption{%
        ...
        The other SYCL implementations are not under active development anymore or have been officially discontinued (ComputeCpp). 
...
Since SYCL is also a standard like \gls{opencl}, many different implementations exist, as shown in \autoref{tab:sycl_implementations}. 
...
SYCL also provides a standard way to ensure interoperability with the actual backend implementations. 
...
The SYCL specification also mandates two different \glspl{api} to interact with memory allocation and movement: the high-level buffer-accessor model and the low-level pointer-based \gls{api}. 
While the former may be more convenient since it automatically handles read-write dependencies, only the latter will be used in this dissertation since it is generally more performant~\cite{crisci_sycl-bench_2024}. 
SYCL internally builds a task graph where all queue operations, like memory copies or kernel launches, are enqueued. 
Using SYCL events with the pointer-based \gls{api}, dependencies can be specified to be sure that, for example, two kernels are always executed one after another. 
...
It should be noted that this asynchronous processing of the task graph can be disabled using the \code{sycl::property::queue::in_order} property provided to the SYCL queue constructor, which implicitly serializes all SYCL runtime calls.
Additionally, SYCL provides a portable way to gather device-specific information, such as the device name, number of cores, or available memory. 
...
Here, we can also see the original close relationship between SYCL and \gls{opencl}: in the \gls{opencl} example in \autoref{code:opencl_example}, we also had to create a \code{cl_command_queue}. 
However, we can also see the benefits of the more modern SYCL \gls{api}, since creating a \code{sycl::queue} can be done in a single line of code, whereas we needed six lines for \gls{opencl} in \autoref{code:opencl_example}.
After that, as was the case with \gls{cuda}, we have to allocate memory on the device and copy the data to it. 
In SYCL, however, we can do this with a more \cpp like \gls{api}: in lines 3-6, we can see that we do not have to calculate our sizes in bytes, removing an additional point of failure. 
...
\AdvantagesAndDisadvantages{sycl}{
    ...
    \item builtin backend interoperability
}{
