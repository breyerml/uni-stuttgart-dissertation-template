...
Therefore, according to \autoref{def:peak_flops}, the theoretical peak \gls{flops} of the example machine's dual-socket \glspl{cpu} are: $2 \cdot 8 \cdot 1 \cdot 4.1 \cdot 24 \cdot 2 = \SI{3148.8}{\giga\flops}$.
...
It contains two floating point operations, a multiplication and an addition, which must be counted as one instruction since the  \gls{amd} EPYC 9274F \gls{cpu} supports \gls{fma} operations. 
Therefore, the number of floating point operations of the TRIAD function is directly based on the size of the vectors. 
Running the TRIAD implementation from the BabelStream benchmark suite~\cite{deakin_evaluating_2018} with the \gls{openmp} implementation setting \code{export OMP_NUM_THREADS=48;export OMP_PLACES=cores} to make sure that only physical cores are used and a vector size of $\num{1e10}$ on our dual-socket \gls{amd} EPYC 9274F \glspl{cpu} node results in an average runtime of $\SI{0.46969}{\second}$ for five repetitions.
This corresponds to a performance of $\SI{21.29}{\giga\flop}$. 
The same code profiled with likwid-perfctr's \code{FLOP_DP} metric, augmented with likwid's marker \gls{api}, reports $\SI{19.7}{\giga\flop}$, which is a discrepancy of only $\SI{4}{\percent}$ compared to the hand-counted variant. 
