...
The first version of the performance portability metric developed by \citet{pennycook_metric_2016} was developed as a preprint in \citeyear{pennycook_metric_2016} on arXiv. 
...
\end{quotebox}
In contrast to the previous informal definitions, this one is more objective since it specifies that the \textit{performance efficiency} measurement, i.e., the ratio of some observed performance with respect to some known, achievable level of performance, should be used to determine whether an application has \enquote{good} performance or not. 
...
\begin{definition}[Architectural Efficiency]\label{def:arch_eff}
    The achieved performance as a fraction of the \enquote{peak} theoretical hardware performance, e.g., the achieved \gls{flops} or memory bandwidth.
\end{definition}
...
The \pp metric has the nice property that, per construction, it is in the interval $\interval{0}{1}$ where $0$ means the application did not run on all hardware platforms and $1$ means the best possible performance, according to the chosen performance efficiency, on all hardware platforms could be achieved. 
...
This \autoref{def:pp} laid the foundation for many researches in the following years.
