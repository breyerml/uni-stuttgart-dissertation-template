...
The hws library is written in modern \cpp{17} and can be used to retrieve hardware-related information from user code using its library interface~\cite{breyer_hws_2024}. 
...
All related functions are available in the custom \code{hws} namespace. 
...
At the start of the sampling process, it is checked which available hardware samples are supported on the current hardware platform. 
...
These events can mark special execution points in a program, such as the start of a solver iteration. 
